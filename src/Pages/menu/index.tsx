...
export default function Page() {
  ...
  const handleCategoria = ({ categoryIndex, category }: { categoryIndex: number; category: MenuCategory }) => {
    if (categoryIndex !== category.id) {
      setCategoryIndex(category.id)
      if (!category.hasSubCategory) setPagina({ categoryId: category.id, subcategoryId: 0 })
    }
  }

  const handleSubCategoria = ({ categoryIndex, subCategory }: { categoryIndex: number; subCategory: MenuSubCategory }) => {
    setPagina({
...
"
      ...
      <main className={styles.main}>
        <div className={styles.titulo}>
          ...
          <p>En nuestro Bowling ofrecemos una variedad de comidas y bebidas deliciosas para una experiencia completa.</p>
        </div>
      </main>
      <article className={styles.carta}>
                ...
                <ul>
                  {categories[categoryIndex].subCategories.map((subCategory) => (
                    <li
                      ...
                      onClick={() => handleSubCategoria({ categoryIndex, subCategory })}
                    >
        ...
        <section className={styles.cartaInfo}>
          {currentCategory.hasSubCategory
            ? currentCategory.subCategories[pagina.subcategoryId].items.map((item) => <ItemCarta key={item.name} item={item} />)
            : currentCategory.items.map((item) => <ItemCarta key={item.name} item={item} />)}
          <Marcas />
        </section>
      </article>
    </>
